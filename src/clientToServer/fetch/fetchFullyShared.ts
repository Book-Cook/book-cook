...
export const useCheckFullyShared = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/user/sharedWithUsers?owner=${encodeURIComponent(sanitizedOwnerEmail)}&user=${encodeURIComponent(sanitizedCurrentUserEmail)}`
      );
      if (!response.ok) {
        return false;
      }
    ...
    enabled: Boolean(ownerEmail && currentUserEmail),
  });
};
