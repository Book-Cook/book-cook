      ...
      if (plan.meals.timeSlots && Array.isArray(plan.meals.timeSlots)) {
        plan.meals.timeSlots.forEach((slot) => {
          slot.meals.forEach((meal) => {
            if (
              meal.recipeId &&
              !seenRecipeIds.has(meal.recipeId) &&
              meal.recipe
            ) {
              const mealDateTime = new Date(
                `${plan.date}T${slot.time}:00.000Z`,
              );
              ...
              if (recipe?.title) {
                allMeals.push({
                  recipe: {
                    ...
                    isPublic: false,
                    isPast:
                      mealDateTime.getTime() + 60 * 60 * 1000 <
                      currentTime.getTime(), // 1 hour buffer
                  },
      ...
      legacyMealTypes.forEach((mealType) => {
        ...
        if (
          ...
          if (meal.recipeId && !seenRecipeIds.has(meal.recipeId)) {
            ...
            const mealDateTime = new Date(
              `${plan.date}T${defaultTime}:00.000Z`,
            );
            const recipe = meal.recipe;
