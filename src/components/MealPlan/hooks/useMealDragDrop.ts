...
export function useMealDragDrop(handlers: MealDragDropHandlers, view: string) {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      // Handle meal card dropped on a different time slot (move)
      if (
        ...
        if (sourceDate !== targetDate || sourceTime !== targetTime) {
          handlers.moveMealMutation.mutate({
            sourceDate,
            sourceTime,
      ...
      if (recipe && dropTarget?.date) {
        ...
        // If it's a month view day, week view drop zone, or general day drop - show time picker
          ...
          dropTarget.type === "month-day" ||
          dropTarget.type === "week-day" ||
          view === "week" ||
          view === "month"
        ) {
